...
const AddVoucher = ({ handleAdd, formValues, setFormValues, addPopupOpen, addPopup, addPopupClose, itemToEdit, handleSubmitEdit, selectedValue, handleSelectChange, handleCategoryTypeChange, selectCategory }) => {
            ...
            <div className='pb-6 pt-3 md:pt-0'>
                <div className='flex justify-between items-center'>
                    ...
                    <button onClick={addPopupOpen} className='flex gap-2 py-1 md:py-2 px-2 md:px-4 bg-primary text-white items-center'>
                        ...
                        <p className={`${banglaFontClass} text-base md:text-lg`}>{t('New')} {t('VoucherCategory')}</p>
                    </button>
                </div>
            </div>
            {
                        ...
                        <OutsideClickHandler onOutsideClick={addPopupClose}>
                            <form onSubmit={itemToEdit ? handleSubmitEdit : handleAdd} className='w-[320px] md:w-[1000px] bg-white dark:bg-graydark p-5 rounded-md animate__animated animate__zoomIn animate__faster'>
                                <div className='flex justify-between items-center border-b border-stroke pb-2'>
                                    <div>
                                        {itemToEdit ? (
                                            <p className={`${banglaFontClass} font-bold text-xl`} >{t('VoucherCategory')} {t('Edit')}</p>
                                        ) : (
                                            <p className={`${banglaFontClass} font-bold text-xl`} >{t('New')} {t('VoucherCategory')}</p>
                                        )}
                                    </div>
                                    <div>
                                    ...
                                    <div className='flex flex-col md:flex-row gap-2 md:gap-6 justify-between py-2 pt-5'>
                                        <div className='w-full md:w-1/2'>
                                            ...
                                            <select
                                                ...
                                                <option value="" className={`${banglaFontClass}`}>{t('selectType')}</option>
                                                <option value="Income" className={`${banglaFontClass}`}>{t('Income')}</option>
                                                <option value="Expense" className={`${banglaFontClass}`}>{t('Expense')}</option>
                                            </select>
                                        </div>
                                        <div className='w-full md:w-1/2'>
